<template>
    <admin-layout>
        ...
        <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 pr-10 lg:px-8 w-full">
            <div
                ...
                <div class="flex justify-between">
                    ...
                    <div class="flex justify-center">
                        <inertia-link as="button" :href="route('admin.movie.import')"
                            class="px-3 py-2 my-auto border-blue-500 border text-blue-500 rounded transition duration-300 hover:bg-blue-700 hover:text-white focus:outline-none mr-2">Import movie</inertia-link>
                        <inertia-link as="button" :href="route('admin.movie.create')"
                            class="px-3 py-2 my-auto border-green-500 border text-green-500 rounded transition duration-300 hover:bg-green-700 hover:text-white focus:outline-none">Add movie</inertia-link>
                    </div>
                </div>
            </div>
            <div
                        ...
                        <tr v-for="movie in movies.data" :key="movie.id">
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                                <div class="flex items-center">
                                    <div>
                                        <div class="text-sm leading-5 text-gray-800">{{ movie.id }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                                <div class="text-sm leading-5 text-blue-900">{{ movie.title }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                                <div class="text-sm leading-5 text-blue-900">{{ movie.rating }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                                <div class="text-sm leading-5 text-blue-900">{{ movie.duration }}</div>
                            </td>
                            <td
                            ...
                            <td
                                ...
                                <inertia-link as="button" :href="route('admin.movie.show', {movie: movie.id})"
                                    class="px-3 py-2 border-indigo-500 border text-indigo-500 rounded transition duration-300 hover:bg-indigo-700 hover:text-white focus:outline-none">View</inertia-link>
                                <inertia-link as="button" :href="route('admin.movie.edit', {movie: movie.id})"
            ...
            <pagination :data="movies" activeClass="bg-blue-500" />
        </div>
    </admin-layout>
</template>
            ...
            deleteMovie(id)
            {
                this.$swal({
                    ...
                    if (result.isConfirmed) {
                        this.$inertia.delete(route('admin.role.delete', {role:id}), {
                            preserveScroll: true,
                            onSuccess: () => {
                                this.$swal(
                                    'Deleted!',
