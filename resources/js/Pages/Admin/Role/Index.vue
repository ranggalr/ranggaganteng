        ...
        <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 pr-10 lg:px-8 w-full">
            <div
                ...
                <div class="flex justify-between">
                    ...
                    <inertia-link as="button" :href="route('admin.role.create')"
                        class="px-3 py-2 my-auto border-green-500 border text-green-500 rounded transition duration-300 hover:bg-green-700 hover:text-white focus:outline-none">Add role</inertia-link>
                </div>
            </div>
            <div
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="bg-white">
                        <tr v-for="role in roles.data" :key="role.id">
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
                                <div class="text-sm leading-5 text-blue-900">{{ role.name }}</div>
                            </td>
                            <td
                                class="px-6 py-4 whitespace-no-wrap border-b border-gray-500 text-blue-900 text-sm leading-5">
                                {{ moment(role.created_at).format('MMMM Do YYYY, h:mm:ss a') }}</td>
                            <td
                                class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                                <inertia-link as="button" :href="route('admin.role.edit', {role: role.id})"
            ...
            <pagination :data="roles" activeClass="bg-blue-500" />
        </div>
    </admin-layout>
</template>
